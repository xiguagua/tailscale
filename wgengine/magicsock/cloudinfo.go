...
func (ci *cloudInfo) getAWSMetadata(ctx context.Context, token, path string) ([]string, error) {
	req, err := http.NewRequestWithContext(ctx, "GET", "http://"+cloudenv.CommonNonRoutableMetadataIP+path, nil)
	if err != nil {
...
func (ci *cloudInfo) getAWS(ctx context.Context) ([]netip.Addr, error) {
	...
	req, err := http.NewRequestWithContext(ctx, "PUT", "http://"+cloudenv.CommonNonRoutableMetadataIP+"/latest/api/token", nil)
	if err != nil {
